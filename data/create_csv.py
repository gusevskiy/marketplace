...
brand = np.random.choice(["Кузница A", "Кузница B", "Кузница C", "Кузница D"],
                         50)
material = np.random.choice(
    ["Нержавеющая сталь", "Углеродистая сталь", "Дамаск"], 50)
blade_length = np.random.uniform(5, 15,
                                 50).round(1)  # Случайная длина от 5 до 15 см
handle_material = np.random.choice(["Дерево", "Пластик", "Резина", "Металл"],
                                   50)
weight = np.random.uniform(50, 300,
                           50).round(1)  # Случайный вес от 50 до 300 грамм
