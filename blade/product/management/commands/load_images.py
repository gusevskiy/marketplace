...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not os.path.exists(image_folder):
            self.stdout.write(self.style.ERROR(f"Папка {image_folder} не существует"))
            return

        # Перебираем файлы в папке
        for file_name in os.listdir(image_folder):
            if file_name.endswith(('.jpg', '.jpeg', '.png')):  # Проверяем формат
                # Получаем ID продукта из имени файла (например, 1.jpg → id=1)
                product_id = os.path.splitext(file_name)[0]
                try:
                    product = Product.objects.get(id=product_id)
                except Product.DoesNotExist:
                    self.stdout.write(self.style.WARNING(f"Продукт с id={product_id} не найден"))
                    continue
